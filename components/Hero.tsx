...
import { Arrow, Sparkle, Code, Wrench } from "./Icons";
...
export default function Hero() {
  return (
    <section id="home" className="section-frame relative overflow-hidden min-h-[640px]">
      <RevealSection>
        {/* ── Meta row ── */}
        <div className="reveal meta flex flex-wrap items-center gap-3 text-link sm:gap-4">
        ...
        <div className="mt-6 grid gap-6 lg:grid-cols-[1fr_280px]">
          ...
          <div className="relative">
            {/* Headline */}
            <h2 className="reveal font-display uppercase leading-[0.78] tracking-[-0.035em] text-[clamp(3rem,15vw,11rem)]">
              {site.brand}
            </h2>
            <p className="reveal reveal-delay-1 font-display uppercase tracking-[0.08em] text-[clamp(0.85rem,2.5vw,1.6rem)] text-ink/60 mt-2">
              Ankit Kush
            </p>
            ...
            <div className="relative z-20 mt-5">
              <p className="reveal reveal-delay-1 max-w-sm text-[14px] leading-relaxed sm:text-[15px]">
                {site.tagline.pre}
                <span className="font-mono text-link">{site.tagline.accent}</span>
                {site.tagline.post}
              </p>

              <div className="reveal reveal-delay-2 mt-6 flex flex-col gap-3 sm:mt-7 sm:flex-row sm:flex-wrap sm:gap-4">
            ...
            <div className="mt-8 lg:hidden">
              <Image
                ...
                width={400}
                height={520}
                className="mx-auto h-auto max-h-[340px] w-auto object-contain character-glow sm:max-h-[400px]"
                priority
              />
            </div>

            {/* Mobile cluster items */}
            <div className="mt-6 flex flex-wrap items-center gap-3 lg:hidden sm:gap-4">
              <div className="relative h-16 w-16 shrink-0 sm:h-20 sm:w-20">
                <AnalogClock />
                <div className="meta absolute -bottom-4 left-1/2 -translate-x-1/2 text-link text-[8px]">IST</div>
              </div>
              <div className="border border-link px-3 py-2 text-center shrink-0 sm:px-4 sm:py-3">
                <Sparkle className="mx-auto h-3 w-3 text-link" />
                <div className="section-kicker mt-1 text-[8px] sm:text-[9px]">{site.role}</div>
              </div>
              <div className="border border-ink bg-signal px-3 py-2 shadow-hard-sm shrink-0 sm:px-4" style={{ transform: "rotate(-3deg)" }}>
                <div className="font-mono text-[10px] font-medium leading-tight tracking-widest text-ink sm:text-[11px]">
                  {site.sticker[0]}<br />{site.sticker[1]}
                </div>
              </div>
              <div className="border border-ink bg-ink px-3 py-2 text-paper shrink-0 sm:px-4 sm:py-3">
                <div className="font-display text-base sm:text-lg">{site.kanji} ARISE</div>
                <div className="mt-1 flex gap-1">
                  {[0, 1, 2, 3].map((i) => (<span key={i} className="h-4 w-0.5 bg-paper" />))}
                </div>
                <div className="meta mt-2 text-[7px] text-paper/70 sm:text-[8px]">DEVLOG / VER.01</div>
              </div>
            </div>
          </div>

          {/* ── Right column: Status data panels ── */}
          <div className="reveal reveal-delay-3 flex flex-col gap-3 self-start sm:gap-4">
            {site.cards.map((c) => {
              ...
              return (
                <div key={c.label} className="data-panel">
                  ...
                  <div className="data-panel__body flex-col items-start">
                    <div className="meta text-[12px] font-bold text-ink sm:text-[13px]">{c.title}</div>
                    <div className="mt-1 font-mono text-[10px] text-ink/60 sm:text-[11px]">{c.sub}</div>
                  </div>
                </div>
              );
            })}
